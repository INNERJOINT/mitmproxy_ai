...
class TestWebAuth:
    def test_no_auth_by_default(self):
        a = webaddons.WebAuth()
        with taddons.context(webaddons.WebAddon(), a):
            assert a.is_valid_password("")
            assert a.is_valid_password("anything")
            assert "token" not in a.web_url

    def test_token_auth(self):
        a = webaddons.WebAuth()
        with taddons.context(webaddons.WebAddon(), a) as tctx:
            tctx.options.web_auth = True
    ...
    def test_web_url_with_auth(self, caplog, web_host, web_port, expected_web_url):
        a = webaddons.WebAuth()
        with taddons.context(webaddons.WebAddon(), a) as tctx:
            tctx.options.web_auth = True
            tctx.options.web_host = web_host
